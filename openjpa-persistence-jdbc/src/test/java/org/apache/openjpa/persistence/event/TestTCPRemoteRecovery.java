...
public class TestTCPRemoteRecovery extends AbstractTestCase {
	...
	private static final Logger logger = Logger.getLogger(TestTCPRemoteRecovery.class.getCanonicalName());
    ...
    public void testReceiverRecovers() {
        ...
        logger.fine("Testing scenario where receiver is failed, then recovered after two "
        		+ "timeouts all the while with the sending pm continuing to send.");
        ...
        logger.fine("You should now see 1 WARN triggered as the sender-pmf tries to send.");
        ...
        logger.fine("Now waiting a recoverytime so that the sender");
        logger.fine("will resume trying to connect to the receiver.");
        pause(15.1);
